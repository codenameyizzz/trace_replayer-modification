...
void *perform_io() {
    ...
    while (1) {
	  ...
	  if ((rand() % 100) < ECC_ERROR_PCT) {
	    int retries = (rand() % MAX_ECC_RETRIES) + 1;
	    for (int i = 0; i < retries; i++) {
	      usleep( (rand() % MAX_ECC_DELAY_MS) * 1000 );
	    }
	  }
	#endif
	    ...
	    if ((rand() % 100) < INJECT_PCT) {
	        ...
	        useconds_t delay_us = f * chunks * 1000;
	        usleep(delay_us);
	    }
	#endif
...
// === Unified Fault Injection Entry Point ===
void *perform_io() {
#if defined(FAULT_FTCX)
// -- FTCX firmware tail latency logic --

#elif defined(FAULT_GC)
// -- GC slowness fault logic --
int64_t cur_idx;
    static __thread int read_counter = 0;
    static __thread int gc_slow_mode = 0;
    ...
    while (1)
    {
        ...
        else if (reqflag[cur_idx] == READ)
        {
            ...
            // Trigger slow mode based on probability 
            if (!in_gc_slow && (rand() % 100) < gc_injection_prob)
            { // 0.5% chance
                in_gc_slow = 1;
                gc_slow_remaining = 10; // next 10 I/Os get delay
                printf("[gc] Entering slow mode at index %ld\n", cur_idx);
            }

            if (in_gc_slow)
            {
                usleep(read_injection_delay_us); // e.g., 300000 us
                gc_slow_remaining--;
...
int main(int argc, char **argv)
{
    ...
    // Default values
    int read_injection_delay_us = 300000;
    int gc_injection_prob = 5;
    ...
    while ((opt = getopt(argc, argv, "t:p:")) != -1)
    {
        ...
        {
        case 't':
            read_injection_delay_us = atoi(optarg);
            break;
        case 'p':
            gc_injection_prob = atoi(optarg); // misal -p 10 artinya 10%
            break;
        default:
            fprintf(stderr, "Usage: %s [-t delay_us] [-p probability] /dev/nvmeXnY tracefile logfile\n", argv[0]);
            exit(EXIT_FAILURE);
        }
    }
    ...
    sprintf(tracefile, "%s", argv[optind + 1]);
    sprintf(logfile, "%s", argv[optind + 2]);

    printf("Device ==> %s\n", device);
    printf("Trace ==> %s\n", tracefile);
    printf("Logfile ==> %s\n", logfile);
    printf("GC injection probability: %d%%\n", gc_injection_prob);
    printf("Injection delay: %d us\n", read_injection_delay_us);
    ...
    free(buff);
    return 0;
#elif defined(FAULT_RAID)
// -- RAID tail amplification fault logic --
int64_t cur_idx;
    int mylatecount = 0;
    int myslackcount = 0;
    struct timeval t1, t2;
...
int main(int argc, char **argv)
{
    ...
    long slow_offset_start = 0;
    long slow_offset_end = 0;
    int request_type = 2; // 0 = WRITE, 1 = READ, 2 = BOTH

    if (argc < 4)
    {
        printf("Usage: ./io_replayer /dev/nvme0n1 tracefile logfile [options]\n");
        printf("Options:\n");
        printf("  -d <delay_ms>        Inject delay in ms\n");
        printf("  -m <offset_start>    Start offset for injection\n");
        printf("  -x <offset_end>      End offset for injection\n");
        printf("  -r <type>            Request type (0=WRITE, 1=READ, 2=BOTH)\n");
        exit(1);
    }
    ...
    return 0;
#else
    printf("Error: No fault injection macro defined. Use -DFAULT_XXX at compile time.\n");
    exit(1);
#endif
    return NULL;
}
