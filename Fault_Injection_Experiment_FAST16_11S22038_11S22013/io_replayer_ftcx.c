...
struct fault_config
{
    int target_disk_id;
    int probability;
    int slow_mode_duration;
    int delay_min_us;
    int delay_max_us;
};

struct fault_config g_fault_cfg; // Global instance
...
void *perform_io()
{
    ...
    while (1)
    {
        ...
        // configureable
        if (!in_slow_mode &&
            logical_disk_id == g_fault_cfg.target_disk_id &&
            (rand() % 100) < g_fault_cfg.probability)
        {

            in_slow_mode = 1;
            slow_mode_remaining = g_fault_cfg.slow_mode_duration;
        ...
        {
            int delay_range = g_fault_cfg.delay_max_us - g_fault_cfg.delay_min_us;
            int delay_us = g_fault_cfg.delay_min_us + (rand() % (delay_range + 1));
...
int main(int argc, char **argv)
{
    ...
    int opt;
    while ((opt = getopt(argc, argv, "d:p:n:m:x:")) != -1)
    {
        switch (opt)
        {
        case 'd':
            g_fault_cfg.target_disk_id = atoi(optarg);
            break;
        case 'p':
            g_fault_cfg.probability = atoi(optarg);
            break;
        case 'n':
            g_fault_cfg.slow_mode_duration = atoi(optarg);
            break;
        case 'm':
            g_fault_cfg.delay_min_us = atoi(optarg);
            break;
        case 'x':
            g_fault_cfg.delay_max_us = atoi(optarg);
            break;
        default:
            fprintf(stderr, "Usage: %s -d <disk_id> -p <probability> -n <slow_n_io> -m <min_delay> -x <max_delay>\n", argv[0]);
            exit(EXIT_FAILURE);
        }
    }
    ...
    if (argc - optind != 3)
    {
        fprintf(stderr, "Usage: %s [options] <device> <tracefile> <logfile>\n", argv[0]);
        exit(EXIT_FAILURE);
    }
    else
    {
