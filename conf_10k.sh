...
firmware_tail)
  ...
  read -p "Total logical disks [default=16]: " FTCX_MAX_DISKS
  FTCX_MAX_DISKS=${FTCX_MAX_DISKS:-16}
  EXTRA_DEFS="
      -DFAULT_FW_TAIL \
      -DTARGET_DISK_ID=${TARGET_DISK_ID} \
      -DINJECT_PCT=${INJECT_PCT} \
      -DFTCX_SLOW_IO_COUNT=${FTCX_SLOW_IO_COUNT} \
      -DDELAY_MIN_US=${DELAY_MIN_US} \
      -DDELAY_MAX_US=${DELAY_MAX_US} \
      -DFTCX_MAX_DISKS=${FTCX_MAX_DISKS}"
  LOGTAG="ftcx_d${TARGET_DISK_ID}_p${INJECT_PCT}_n${FTCX_SLOW_IO_COUNT}_u${DELAY_MIN_US}-${DELAY_MAX_US}"
  ;;
raid_tail)
  read -p "Injection delay (ms): " INJECTION_DELAY
  read -p "Start offset (bytes): " INJECTION_START
  read -p "End offset (bytes): " INJECTION_END
  echo "Request type: 0 = WRITE, 1 = READ, 2 = BOTH"
  read -p "Injection type (0/1/2): " INJECTION_TYPE

  EXTRA_ARGS="-d ${INJECTION_DELAY} -m ${INJECTION_START} -x ${INJECTION_END} -r ${INJECTION_TYPE}"

  LOGTAG="raid_d${INJECTION_DELAY}_m${INJECTION_START}_x${INJECTION_END}_r${INJECTION_TYPE}"
  ;;
  gc_slowness)
    read -p "Injection probability (%) [0–100]: " GC_PROB
    read -p "Read delay per I/O (us): " GC_DELAY_US

    EXTRA_DEFS="-DFAULT_GC_SLOW"
    EXTRA_ARGS="--gc-prob ${GC_PROB} --gc-delay ${GC_DELAY_US}"
    LOGTAG="gc_p${GC_PROB}_d${GC_DELAY_US}"
    ;;
esac
