...
// global variable
int fs_delay_us = 0;
int gc_interval_ms = 200;
int last_gc_us = 0;
int fsdelay_burst_len = 5;
int fsdelay_active = 0;
int fsdelay_left = 0;
...
void *perform_io()
{
    ...
    while (1)
    {
        ...
        else if (reqflag[cur_idx] == READ)
        {
            ...
            int64_t now_us = now.tv_sec * 1000000LL + now.tv_usec;
            ...
            if (fs_delay_us > 0)
            {
                ...
                if (fsdelay_active)
                {
                    usleep(fs_delay_us);
                    fsdelay_left--;
                    fprintf(stderr, "[Injected fs delay] ACTIVE #%ld delayed %dus\n", cur_idx, fs_delay_us);
                    if (fsdelay_left == 0)
                        fsdelay_active = 0;
                }
                else if (now_us - last_gc_us >= gc_interval_ms * 1000LL)
                {
                    fsdelay_active = 1;
                    fsdelay_left = fsdelay_burst_len;
                    last_gc_us = now_us;
                    usleep(fs_delay_us);
                    fsdelay_left--;
                    fprintf(stderr, "[Injected fs delay] BURST START #%ld delayed %dus\n", cur_idx, fs_delay_us);
                }
            }
...
int main(int argc, char **argv)
{
    ...
    char *dummy_device_path = NULL;
    int io_type = 1;

    int opt;
    while ((opt = getopt(argc, argv, "f:l:r:d:g:b:y:")) != -1)
    {
        switch (opt)
        {
        case 'f':
        ...
        case 'l':
            ...
            break;
        case 'r':
            io_type = atoi(optarg); // 1 for READ
            break;
        case 'd':
            dummy_device_path = strdup(optarg);
            break;
        case 'g':
            gc_interval_ms = atoi(optarg);
            break;
        case 'b':
            fsdelay_burst_len = atoi(optarg);
            break;
        case 'y':
            fs_delay_us = atoi(optarg);
            break;
        default:
            fprintf(stderr, "Usage: %s -f trace -l logfile -r 1 -d /dev/null -g <ms> -b <len> -y <delay_us>\n", argv[0]);
            exit(EXIT_FAILURE);
        }
    }

    if (!trace_file_path || !log_file_path || !dummy_device_path)
    {
    ...
    {
        sprintf(device, "%s", dummy_device_path);
        printf("Device ==> %s\n", device);
        sprintf(tracefile, "%s", trace_file_path);
        printf("Trace ==> %s\n", tracefile);
        sprintf(logfile, "%s", log_file_path);
        printf("Logfile ==> %s\n", logfile);
    }
    // start the disk part
    fd = open(device, O_DIRECT | O_RDONLY);
    if (fd < 0)
    {
