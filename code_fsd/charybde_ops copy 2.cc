...
int charybde_fsync(const char *path, int datasync, struct fuse_file_info *fi)
{
    ...
    fprintf(stderr, "[DEBUG] fsync(): path=%s, in_flight=%d\n", path, in_flight);

    
    int ret = error_inject(in_flight, path, "fsync");
    if (ret) {
        fprintf(stderr, "[DEBUG] fsync(): error_inject() returned %d, returning early\n", ret);
        in_flight--;
        return ret;
    }

     if (strstr(path, "test.txt") != NULL) {
        fprintf(stderr, "[DEBUG] Injecting delay in fsync(): path=%s\n", path);
        usleep(500000); 
    }

    if (datasync) {
        ret = fdatasync(fi->fh);
        fprintf(stderr, "[DEBUG] fsync(): fdatasync() = %d\n", ret);
        if (ret < 0) {
            in_flight--;
            return -errno;
        }
        ...
        ret = fsync(fi->fh);
        fprintf(stderr, "[DEBUG] fsync(): fsync() = %d\n", ret);
        if (ret < 0) {
            in_flight--;
            return -errno;
        }
    ...
    in_flight--;
    fprintf(stderr, "[DEBUG] fsync(): success\n");
    return 0;
}
