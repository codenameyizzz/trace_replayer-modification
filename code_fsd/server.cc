...
// Return true jika tidak dapat error/delay (lolos), false jika harus inject
static bool get_lucky(int probability)
{
    if (probability <= 0) return true;  // always skip
    if (probability >= 100000) return false; // always inject

    std::random_device rd;
    std::uniform_int_distribution<int> dist(1, 100000);
    return dist(rd) > probability;
}
...
int error_inject(volatile int in_flight, std::string path, std::string method)
{
    ...
    // Check probability *before doing anything*
    if (get_lucky(descr.probability))
        return 0;

    // Log injection
    std::cout << "[INJECT] method=" << method
              << " kill=" << descr.kill_caller
              << " errno=" << descr.err_no
              << " delay_us=" << descr.delay_us
              << std::endl;

    // Apply delay if set
    if (descr.delay_us > 0 && !descr.auto_delay)
    {
        std::cerr << "[INJECT_DELAY] sleeping " << descr.delay_us << "us for method=" << method << "\n";
        std::this_thread::sleep_for(std::chrono::microseconds(descr.delay_us));
    }

    // Simulate kill (optional, never actually kill)
    if (descr.kill_caller && err_no != 0)
    {
        std::cout << "[SKIP_KILL] Simulating kill, but not actually killing.\n";
        return 0;
    }
    ...
    // If no error, just return 0 (after delay)
    return 0;
}
                   ...
                   int32_t delay_us, const bool auto_delay)
    {
        ...
        for (auto method : methods)
        {
            if (is_valid_method(method))
            {
                ...
                std::cout << "[SET_FAULT] method=" << method
                          << " delay_us=" << delay_us
                          << " kill=" << kill_caller
                          << " regexp=" << regexp << std::endl;
            }
        }
    }
