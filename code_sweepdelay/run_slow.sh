...
# 4) Start log capture
docker compose logs -f cassandra_a cassandra_b cassandra_c >"$OUTDIR/docker-logs.txt" 2>&1 &
LOG_PID=$!

# 5) Run cassandra-stress in the *current* node a container
echo ">> Running cassandra-stress for ${DURATION}s…"
docker compose exec cassandra_a bash -lc "/opt/cassandra/tools/bin/cassandra-stress write \
  duration=${DURATION}s \
